     ...
     <!-- tiitle栏 -->
        <div style="padding-bottom:30px">
         ...
         <div class="top-title" style=" margin-top:62px; margin-left:18px; line-height: 50px;">
                <span class="el-icon-info"></span><span style="margin-left:10px">文件管理</span><span>&nbsp;&nbsp;/&nbsp;&nbsp;轮播图管理</span>
         </div>
        </div>
...
// ------------------------------------------------------------弹窗图片处理-------------------------------------------------------------------------------------------------------------------
        ...
        addMost(){
            ...
            this.submitType="add";
            this.upPictureUrl=url.server+url.scInsertByCarousel
            this.upfileList=[]
            this.ifImgChange=false
        ...
        changeRow(index, rows){
            ...
            this.upPictureUrl=url.server+url.scUpdateByCarousel
        },
          ...
          if (valid) {
            ...
            if(this.submitType=="change"){
                ...
                if(this.ifImgChange){     //图片修改
                    ...
                    postRequest(url.scUpdateByCarousel,{
                        sort:this.ruleForm.sort,
                        id:this.changeId,
                        file:""
                    }).then(data=> {
      ...
      beforeAvatarUpload(file) {
        ...
        deletMost(){
            ...
            postRequest(url.scDeleteByCarousel, {
                   ids:chooseStrId,
                   urls:chooseStrUrl
                }).then(data=> {
...
// f分页
      ...
      handleCurrentChange(val) {
        ...
        this.initSelectAll(this.tableData)
         ...
         initSelectAll(pamas){
             postRequest(url.scSelectByCarousel, pamas).then(data=> {
